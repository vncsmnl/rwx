...
pub enum Focusable {
    ...
    OctalInput,
    OwnerInput, GroupInput,
    Recursive,
    ApplyButton, QuitButton,
}
...
impl App {
    ...
    pub fn toggle_current_checkbox(&mut self) {
        match self.focus {
            ...
            Focusable::OwnerExecute => self.permissions.owner.execute = !self.permissions.owner.execute,
            ...
            Focusable::GroupExecute => self.permissions.group.execute = !self.permissions.group.execute,
            Focusable::OthersRead => self.permissions.others.read = !self.permissions.others.read,
            Focusable::OthersWrite => self.permissions.others.write = !self.permissions.others.write,
            Focusable::OthersExecute => self.permissions.others.execute = !self.permissions.others.execute,
    ...
    pub fn validate_owner(&self) -> bool {
        ...
        User::from_name(&self.owner_input).map(|u| u.is_some()).unwrap_or(false)
    }
    ...
    pub fn validate_group(&self) -> bool {
        ...
        Group::from_name(&self.group_input).map(|g| g.is_some()).unwrap_or(false)
    }

    pub fn apply_changes(&mut self) -> Result<(), String> {
        let mode = self.permissions.to_mode();
        
        let uid = if self.owner_input == self.orig_owner {
            None
        } else if self.owner_input.is_empty() {
            None
        ...
        let gid = if self.group_input == self.orig_group {
            None
        } else if self.group_input.is_empty() {
            None
        ...
        if self.recursive && self.is_dir {
            for entry in walkdir::WalkDir::new(&self.target_path) {
                let entry = entry.map_err(|e| format!("Error reading directory structure: {}", e))?;
                apply_to(entry.path()).map_err(|e| format!("Failed to apply to {:?}: {}", entry.path(), e))?;
            }
        } else {
